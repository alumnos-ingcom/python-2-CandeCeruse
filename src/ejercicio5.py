...
def determina_balance(cadena):
    limite = len(cadena)
    corchete_apertura = 0
    corchete_cierre = 0
    posicion = 0
    primer_apertura = cadena.find('[')
    primer_cierre = cadena.find(']')
    posicion_apertura = 0
    posicion_cierre = 0
    ...
    if cadena == '':
        ...
        if primer_cierre < primer_apertura:
            ...
            while limite > 0:
                apertura = cadena.find('[', posicion_apertura)
                cierre = cadena.find(']', posicion_cierre)
                if apertura <= cierre:
                    if cadena[posicion] == '[':
                        corchete_apertura += 1
...
def principal():
    ...
    """
    cadena = str(input("Ingrese juego de corchetes: "))
    resultado = determina_balance(cadena)
    print(resultado)
